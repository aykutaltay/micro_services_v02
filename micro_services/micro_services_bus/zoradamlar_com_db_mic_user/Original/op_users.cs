...
namespace micro_services_bus.zoradamlar_com_db_mic_user
{
    public partial class Op_users
    {
        ...
        public List<users> GetAllusers(string whereclause = "1=1", string DBTYPE = "", string CONNSTR = "", bool ALL=false)
        {
            ...
            if (ALL == false)
            {
                ...
                whereclause += string.Format(" AND {0}=false AND {1}=true AND {2}=true", info.users_deleteduser_id, info.users_users_use,info.users_users_active);
            }
