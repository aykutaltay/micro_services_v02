...
namespace micro_services_bus
{
    ...
    {
        public List<allofusers> l_allofusers (long userID,string DBTYPE, string CONNSTR)
        {
            ...
            if (DBTYPE == AppStaticStr.core_dbTypeMYSQL)
            {
                using (Mysql_dapper db = new Mysql_dapper(connstr: CONNSTR, usetransaction: false))
                {
                    string sql = string.Format(@"
                        ...
                        left join useractivation as usr_act on usr.users_id=usr_act.useractivation_user_id and usr_act.deleteduseractivation_id=false
