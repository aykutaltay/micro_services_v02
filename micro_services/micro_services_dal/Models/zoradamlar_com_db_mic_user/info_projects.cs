using System;
using System.Collections.Generic;
using System.Text;

namespace micro_services_dal.Models.zoradamlar_com_db_mic_user
{
    public class info_projects
    {
        public string projects_projects_id = "projects_id";
        public string projects_projects_name = "projects_name";
        public string projects_projects_database_id = "projects_database_id";
        public string projects_deletedprojects_id = "deletedprojects_id";
        public string projects_maintenanceprojects = "maintenanceprojects";
        public string projects_projects_active = "projects_active";
        public string projects_projects_use = "projects_use";
        public string projects_tablename = "projects";
    }

}
